...
$(document).ready(function () {   
    $('body').on('change', '.medicine-select', function() {
    	debugger;
    	//$(this).parent('td').find('td:eq(1) input').val($(this).val());
    	//$(this).parent('td').find('td:eq(1)').html($(this).val());
        //console.log( $(this).parent('tr').find('td:eq(1)'));
...
function calculate() {
	//var value1 = document.getElementById('show_selected').value;	
	//var value2 = document.getElementById('quantity').value;
	//var price = document.getElementById('price');	
	//var Result = value1 * value2 ;
	//price.value = Result;
	
	/* var value1 = parseInt(document.getElementsByClassName('t1')[0].value);	
	var value2 = parseInt(document.getElementsByClassName('t1')[1].value);
	var price = document.getElementsByClassName('t1')[2];	
	var Result = value1 * value2 ;
	price.value = Result; */
	debugger;
	total =0;
...
<tr>
 <% for(int i=0;i<5;i++){ %>
  ...
  <td>
		...
		<select name="itemName" class="medicine-select">
		    <c:forEach items="${ItemsForBill}" var="product">
		        <option value="${product.unitPrice}">${product.itemName} ${product.unitPrice}</option>
		    </c:forEach>
		</select>
		</spring:bind>
...
<td>	
		<spring:bind path="quantity">
			<div class="form-group ${status.error ? 'has-error' : ''}">
				...
				<div class="col-sm-10">
					<form:input path="quantity" type="number" class="medicine-quantity" placeholder="quantity"/>
